...
移動的邊界處理：拖到影像邊緣就停住，不會被拖出影像外。多選時是**整組一起夾**——
碰到邊界時整組停下，不會出現「碰到的那幾個停住、其他繼續走」而把相對位置弄散。
偵測結果本來就貼邊、已經超出影像的標註不夾，否則一按下去就會被推回來。

> **控制點只屬於選取中的框。** 控制點本來就只畫給選取中的框，熱區跟著限縮才不會
> 出現「看不到卻踩得到」——框重疊時最容易誤把旁邊那個拖變形。代價是要先點選再拖角。
