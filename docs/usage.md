...
- **Ai → Auto Save (if Auto Detect)**（快捷鍵 `a`）：讓 **Auto Detect 純自動產生的結果**也落檔。**需先開啟 Auto Detect 才可勾選**，關閉 Auto Detect 時會一併關閉

> ### Auto Save 不是「自動存檔」的總開關
>
> **手動動過的標註一律會存，跟這個開關無關。** 只要畫過框、拖曳、resize、旋轉、刪除或改過 label，切換檔案、滾輪換圖、播放影片、關閉程式時都會自動儲存 —— Auto Save 關著也一樣。
>
> 它管的是另一件事：**當你什麼都沒動、畫面上只有模型自己偵測出來的框時，那些框要不要落檔。**
>
> | Auto Save | 你動過標註的圖 | 你沒動過、只有偵測結果的圖 |
> |---|---|---|
> | 關閉 | 儲存 | **跳過** |
> | 開啟 | 儲存 | 儲存（影片還會定時抽幀） |
>
> 所以：要**大量收集訓練資料**就開它，模型看到什麼就存什麼，事後再篩；要**人工把關**、只留自己確認過的圖，就關著它，翻過去沒動的圖不會留下任何檔案。
...
- 有播放進度條，可拖曳跳轉
- 按下滑鼠鍵會暫停播放
- 開啟 **Auto Save (if Auto Detect)** 後，播放期間會自動抽幀儲存，檔名為 `{原檔名}_frame{N}`（需先開啟 Auto Detect）。這是 Auto Save 的另一個用途：把影片轉成一批已標好的訓練圖
